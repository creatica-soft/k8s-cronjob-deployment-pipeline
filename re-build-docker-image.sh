#use PROJECTS env var below and if/else conditions to indicate which docker images needed to be rebuilt
DOCKER_SERVER="nexus.lat.internal:8443"
...
for GITREPO in ${GITREPOS}; do
        ...
        for NAMESPACE in ${NAMESPACES}; do
                ...
                for SCRIPT in ${SCRIPTS}; do
                        ...
                        if [[ "${GITREPO}" == "marketing" && "${NAMESPACE}" == "qa" && "${SCRIPT}" == "r" ]]; then
                          PROJECTS="ggplot_test"
                        else PROJECTS=""
                        fi
                        ...
                        for PROJECT in ${PROJECTS}; do
                          ...
                          export PROJECT JOBNAME DOCKER_IMAGE DOCKER_IMAGE_VERSION MIN_CPU MIN_RAM MAX_CPU MAX_RAM CRONJOB_SCHEDULE MAIN_SCRIPT R_LIBRARIES EMAIL_TO EMAIL_CC EMAIL_FROM
                          if [[ ! ${JOBNAME} =~ ^[a-z0-9]([-a-z0-9]*[a-z0-9])?$ ]]; then
                                ...
                                unset LOG_FILE PROJECT JOBNAME DOCKER_IMAGE DOCKER_IMAGE_VERSION MIN_CPU MIN_RAM MAX_CPU MAX_RAM MAIN_SCRIPT R_LIBRARIES CRONJOB_SCHEDULE EMAIL_TO EMAIL_CC EMAIL_FROM DELETE_CRONJOB
                                continue
                          fi
                          echo -e "DATETIME=${DATETIME}\r" >> ${LOG_FILE}
                          echo -e "PROJECT=${PROJECT}\r" >> ${LOG_FILE}
                          ...
                          if [[ ${DELETE_CRONJOB} == "true" ]]; then
                                ...
                                unset LOG_FILE JOBNAME VERSION PROJECT DOCKER_IMAGE DOCKER_IMAGE_VERSION MIN_CPU MIN_RAM MAX_CPU MAX_RAM MAIN_SCRIPT R_LIBRARIES CRONJOB_SCHEDULE EMAIL_TO EMAIL_CC EMAIL_FROM DELETE_CRONJOB
                                continue
                          fi
                          ...
                          if [[ "${VERSION}" != "${DOCKER_IMAGE_VERSION}" ]]; then
                                ...
                                if (( $? == 0 )); then
                                  ...
                                  echo -e "sudo docker login -u qas-datascience-lat ${DOCKER_SERVER}\r" >> ${LOG_FILE}
                          ...
                          unset LOG_FILE JOBNAME DOCKER_IMAGE VERSION PROJECT DOCKER_IMAGE_VERSION MIN_CPU MIN_RAM MAX_CPU MAX_RAM MAIN_SCRIPT R_LIBRARIES CRONJOB_SCHEDULE EMAIL_TO EMAIL_CC EMAIL_FROM DELETE_CRONJOB
                          cd ..
                     done
                     cd ..
                  done
